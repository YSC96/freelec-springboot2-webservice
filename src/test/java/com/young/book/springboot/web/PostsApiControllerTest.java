...
public class PostsApiControllerTest {
    ...
    @Autowired
    private TestRestTemplate restTemplate; //@WebMvcTest 경우 JPA기능이 작동하지 않으므로 대신 사용, 실제서버가 동작하는것처럼 테스트 가능.
    ...
    public void Posts_insert() throws Exception {
        ...
        //when
        ResponseEntity<Long> responseEntity = restTemplate.postForEntity(url,requestDto,Long.class); // POST 요청을 보내고 결과로 ResponseEntity로 반환받음.

        //then
        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(responseEntity.getBody()).isGreaterThan(0L); // 실제 데이터 정보 확인
    ...
    public void Posts_update() throws Exception {
        ...
        //when
        ResponseEntity<Long> responseEntity = restTemplate.exchange(url, HttpMethod.PUT,requestEntity,Long.class); // exchange : 헤더세팅해서 HTTP Method로 요청보내고 ResponseEntity로 반환받음
